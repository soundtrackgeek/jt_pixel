...
Version `0.14.1` refines the complete Import Studio and project-wide Canvas Operations release with naturally editable slice values, visible custom steppers, and a larger responsive studio while retaining transparent PNG placement, sprite-sheet slicing, palette policies, anchored canvas resizing, nearest-neighbor scaling, live previews, atomic Undo/Redo, seamless tile and symmetry editing, the advanced animation timeline, native system-wide screen picker, in-app Pixel Lens, dependable Palette Studio color management, frame-local layer locking, precision drawing, animated GIF, PNG, and sprite-sheet export, configurable canvas views, crash recovery, and the signed desktop update channel:
